...
// The working search UI. Only rendered for paid users and admins — the
// server page gate (./page.tsx) shows PaywallScreen to everyone else.
export default function SearchClient() {
  ...
  // Set when the API returns 403 { error: "paywall" } — only possible if the
  // subscription lapses mid-session (the page gate passed at render time).
  ...
  return (
    <div>
      ...
      {/* Subscription lapsed mid-session — point back to pricing */}
      {paywalled && (
        <div className="rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-8 text-center">
          <div className="flex items-center justify-center h-12 w-12 rounded-xl bg-brand-50 dark:bg-brand-950 mx-auto mb-4">
            <Lock className="h-6 w-6 text-brand-600 dark:text-brand-400" />
          </div>
          <p className="text-base text-gray-700 dark:text-gray-300 mb-4">
            Search needs an active VCE Maths plan.
          </p>
          <Link
            href="/pricing"
            className="inline-flex items-center justify-center rounded-xl bg-brand-600 hover:bg-brand-700 text-white px-5 py-2.5 text-sm lg:text-base font-semibold transition-colors"
          >
            See plans
          </Link>
        </div>
      ...
      {/* Results */}
      {!paywalled && searched && results.length === 0 && (
