...
export default async function HistoryPage() {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-2xl lg:text-3xl font-bold text-gray-900 mb-1">Exam History</h1>
        <p className="text-sm text-gray-500">Track your performance across practice exams.</p>
      </div>

      {totalSessions === 0 ? (
        <div className="rounded-2xl border border-dashed border-gray-200 p-12 text-center">
          <Trophy className="h-10 w-10 text-gray-300 mx-auto mb-3" />
          <p className="text-gray-400 mb-4">No exam sessions yet. Complete a practice exam to see your history.</p>
          <Link
            href="/practice"
        ...
        <>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="rounded-2xl border border-gray-100 bg-white shadow-sm p-5">
              <div className="inline-flex rounded-xl bg-brand-50 p-2.5 mb-3">
            ...
            <div className="rounded-2xl border border-gray-100 bg-white shadow-sm p-5">
              <div className="inline-flex rounded-xl bg-green-50 p-2.5 mb-3">
            ...
            <div className="rounded-2xl border border-gray-100 bg-white shadow-sm p-5">
              <div className="inline-flex rounded-xl bg-purple-50 p-2.5 mb-3">
                <Clock className="h-5 w-5 text-purple-600" />
              </div>
              <p className={cn("text-2xl font-bold", recentTrend >= 0 ? "text-green-600" : "text-red-600")}>
                {recentTrend >= 0 ? "+" : ""}{recentTrend}%
              </p>
              <p className="text-sm text-gray-500">Trend (first→latest)</p>
            </div>
          </div>
          ...
          {modeMap.size > 1 && (
            <div className="rounded-2xl border border-gray-100 bg-white shadow-sm p-6">
              <h2 className="text-sm font-semibold text-gray-500 uppercase tracking-widest mb-4">By Exam Type</h2>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                {Array.from(modeMap.entries()).map(([mode, data]) => (
                  <div key={mode} className="rounded-xl bg-gray-50 p-4">
          ...
          <div className="rounded-2xl border border-gray-100 bg-white shadow-sm p-6">
            <h2 className="text-sm font-semibold text-gray-500 uppercase tracking-widest mb-4">Score Trend</h2>
          ...
          {/* Session list */}
          <div>
            <h2 className="text-sm font-semibold text-gray-500 uppercase tracking-widest mb-4">Recent Sessions</h2>
                ...
                >
                  <div className="flex items-center gap-4">
                    <div className={cn(
                      ...
                      s.score >= 80 ? "bg-green-50 text-green-600" :
                      s.score >= 50 ? "bg-yellow-50 text-yellow-600" :
                      "bg-red-50 text-red-600"
                    )}>
                      {Math.round(s.score)}%
                    </div>
                    <div>
                      <p className="text-sm font-semibold text-gray-800">{s.mode}</p>
