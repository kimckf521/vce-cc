...
interface ReviewProps {
  ...
  /**
   * Optional override for the "Back to history" link target. Defaults to
   * the global `/history` list. Subject-scoped routes (e.g.
   * `/vce/foundation/history/[id]`) pass their own list URL so the back
   * link stays inside the same namespace.
   */
  backHref?: string;
}
...
/**
 * Practice-session review body. Rendered by both the global `/history/[id]`
 * route and the subject-scoped `/[curriculum]/[subject]/history/[id]` route
 * (the latter passes its own `backHref`). Lives outside `page.tsx` because
 * Next.js page files may only export a fixed set of names.
 */
export default async function HistorySessionReview({
  params,
  backHref = "/history",
}: ReviewProps) {
  ...
  return (
    <div className="space-y-8">
      {/* Header */}
      <div>
        <BackLink href={backHref} label="Back to history" className="mb-4" />
