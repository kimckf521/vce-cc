...
import { VCAA_DIFFICULTY_DIST } from "@/lib/exam-config";
...
// locked, and the setup page just confirms what the student is about to sit.
export default function Exam2ABSetupForm({ topics, title }: Props) {
  const router = useRouter();
  // Route into THIS subject's session page, not a hardcoded Methods URL.
  ...
  function handleStart() {
    ...
    // Even split across the current subject's topics (4/5/6 for
    // Methods/General/Specialist) so every topic is represented.
    const evenSplit = evenTopicSplit(topics.length).join(",");
    const diff = [...VCAA_DIFFICULTY_DIST].join(",");
    const url = `${practiceHome}/session?mode=exam2ab&version=exam&countA=20&countB=${countB}&dist=${evenSplit}&distB=${evenSplit}&diff=${diff}&solutions=${showSolutions ? "1" : "0"}${timerParam}`;
    router.push(url);
  }
  ...
  return (
    <div className="space-y-8 lg:space-y-10">
      ...
      {/* Heading */}
      <h1 className="text-2xl lg:text-3xl font-bold text-gray-900 dark:text-gray-100">{title ?? "Exam 2A & 2B practice"}</h1>
