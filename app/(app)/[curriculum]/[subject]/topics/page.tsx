...
export default async function TopicsPage({ params }: PageProps) {
  ...
  const subjectBase = `/${curriculumSlug}/${subjectSlug}`;
  ...
  const topics = await prisma.topic.findMany({
    where: { subject: { slug: dbSubjectSlug } },
    orderBy: { order: "asc" },
    select: {
      id: true,
      name: true,
  ...
  if (user && !isAdmin) {
    await Promise.all(
      topics.map(async (t) => {
        const access = await canAccessTopic(user.id, t.slug, dbSubjectSlug);
  ...
  return (
    <div>
      ...
      <p className="text-gray-500 dark:text-gray-400 lg:text-base mb-8">
        Browse past exam questions organised by {subjectDisplayName} syllabus topic.
      </p>
        ...
        {topics.map((topic) => {
          ...
          return (
            <div key={topic.id} className="rounded-2xl bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 shadow-sm overflow-hidden">
              {/* Topic header */}
              <LockedTopicLink
                href={`${subjectBase}/topics/${topic.slug}`}
                locked={isLocked}
                    ...
                    return (
                      <div key={sub.id} className="group relative">
                        {isLocked ? (
                          ...
                          <Link
                            href={`${subjectBase}/topics/${topic.slug}?subtopic=${sub.slug}`}
                            className={cardClass}
                          >
                            {cardInner}
                          </Link>
                        ...
                        {/* Hover tooltip — shows description (or "VCE Maths plan required" for locked) */}
